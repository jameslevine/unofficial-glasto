...
- **Task:** None — Phase 9.6 complete (multi-pin Cognito sync with real-time push) and verified end-to-end on deployed dev.
- **Started:** —
- **Context:** Last working session shipped real-time pin sync: pins store now tracks `dirty: Record<string, true>`, `usePinsSync` subscribes to the store and debounces a `POST /v1/me/pins/sync` 400ms after each change. Persist version bumped to v2 with a migration that marks all existing records dirty so they push on first load. Smoke-tested on `https://d5zgsiw27b3ju.cloudfront.net/map`: long-press → drop → DDB row → edit → DDB updated → delete → DDB tombstone, all without a page reload.
...
| 2026-05-23 | M9.6 smoke test — full /v1/me/pins round-trip on deployed CF: drop, edit, delete tombstone all in DDB  | Cognito user `glasto-test@example.com`; UID `3438a458-…0335`           |
| 2026-05-24 | M9.6 polish — pins store v2 with `dirty: Record<id, true>`; `usePinsSync` subscribes + debounces 400ms | Real-time push without reload; v1→v2 migration marks all records dirty |
